...
# Purpose     : Combined AEB-focused perception for Indian unstructured ODD
# Description : Runs YOLOv11 detection + segmentation simultaneously,
#               computes monocular distance, triggers AEB warning logic.
...
# -----------------------------------------------------------------------------
# Real-world object heights for monocular distance estimation
# Source: IS 11231, MoRTH vehicle classification, field measurements
# -----------------------------------------------------------------------------
OBJECT_HEIGHTS_M = {
    'person'    : 1.70,
    'bicycle'   : 1.10,
...
# Camera focal length (pixels) — KITTI reference
# Update with your actual camera calibration
FOCAL_LENGTH_PX = 718.856


class CombinedPipeline:
    """
    Combined perception pipeline for Indian road conditions.

    Pipeline flow:
    1. Segmentation  → colour masks on all objects
    2. Detection     → bounding boxes + class labels on top
    3. Distance      → monocular estimate per object
    4. AEB logic     → warning banner if critical object close
    """

    # AEB distance thresholds (metres)
    AEB_WARNING_M  = 30.0
    AEB_CRITICAL_M = 15.0
    ...
    def __init__(self):
        ...
        self.detector  = YOLODetector(model_path='yolo11s.pt')
        print("[INFO] Loading segmentation model...")
        self.segmentor = YOLOSegmentor(model_path='yolo11s-seg.pt')
    ...
    def estimate_distance(self, class_name, bbox_height_px):
        ...
        Formula: distance = (real_height * focal_length) / bbox_height_px
        """
    ...
    def process_frame(self, frame):
        """
        Process one frame through full pipeline.
        Returns annotated frame + detection list.
        """
        # ── STEP 1: Segmentation (background colour layer) ──────────────────
        annotated = self.segmentor.segment(frame)

        # ── STEP 2: Detection (foreground bounding boxes) ────────────────────
        detections = self.detector.detect(frame)

        # ── STEP 3: Distance estimation + draw boxes ─────────────────────────
        aeb_triggered  = False
        ...
        for det in detections:
            ...
            # Choose box colour
            if det['aeb_critical']:
                color = (0, 0, 255)    # Red — AEB critical
            else:
                color = (0, 255, 0)    # Green — non critical

            # Draw bounding box
            cv2.rectangle(annotated, (x1, y1), (x2, y2), color, 2)

            # Build label with distance
            if distance:
    ...
    def run(self, source, output_path=None, show=True):
        """
        Run combined pipeline on video source.
        Args:
        ...
        print("[INFO] Combined pipeline running...")
        ...
        while True:
            ...
            cv2.putText(
                final, f"FPS      : {fps_val:.1f}",
                (10, panel_y + 20),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6, (0, 255, 255), 2
            )
            cv2.putText(
                final, f"Objects  : {len(detections)}",
                (10, panel_y + 42),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6, (0, 255, 255), 2
            )
            cv2.putText(
                final, f"AEB Alerts: {aeb_count}",
                (10, panel_y + 64),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6, (0, 255, 255), 2
            )
